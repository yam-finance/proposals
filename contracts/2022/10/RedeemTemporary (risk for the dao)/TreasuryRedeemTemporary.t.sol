...
import {TreasuryRedeemTemporary} from "./TreasuryRedeemTemporary.sol";

contract TreasuryRedeemTemporaryTest is YAMTest {
    ...
    function test_redeemer_temporary() public {
        ...
        redeemer = new TreasuryRedeemTemporary(address(testYAM), tokens, MINT_BASE, 365 days);
        ...
        assertEq(testYAM.balanceOf(address(redeemer)), 10000000000000000000000000);
        ...
        // // returnToReserves fails if `_redeemLength` time period not reached
        // ff(100 days);
        ...
        // Reserves should have the tokens left
        assertEq(testYAM.balanceOf(address(reserves)), 10000000000000000000000000);
        assertEq(testYSTETH.balanceOf(address(reserves)), 41000000000000000000);
        assertEq(testWETH.balanceOf(address(reserves)), 213666666666666666667);
        assertEq(testWBTC.balanceOf(address(reserves)), 666666666666666667);
        assertEq(testDPI.balanceOf(address(reserves)), 606666666666666666667);
        assertEq(testYSTETH.balanceOf(address(reserves)), 41000000000000000000);
    }
}
